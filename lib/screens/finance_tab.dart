...
class FinanceTabState extends State<FinanceTab>
  ...
  String _displayRange(DateTimeRange r) =>
      '${_displayDate(r.start)} - ${_displayDate(r.end)}';
  ...
  Future<void> _loadSummary() async {
    final Map<String, String>? rangeMap = _summaryDateRange != null
        ? {
            'start': _formatDate(_summaryDateRange!.start),
            'end': _formatDate(_summaryDateRange!.end),
          }
        : null;
    final res = await _api.getFinanceSummary(
      _filterSummary,
      dateRange: rangeMap,
    );
  ...
  Future<void> _loadEntries() async {
    final Map<String, String>? rangeMap = _entriesDateRange != null
        ? {
            'start': _formatDate(_entriesDateRange!.start),
            'end': _formatDate(_entriesDateRange!.end),
          }
        : null;
  ...
  void _showAddEditDialog({dynamic entry}) {
    ...
    final allocationOptions = _allocations
                    ...
                    children: [
                      ...
                      if (selectedType == 'expense') ...[
                        ...
                        Wrap(
                          spacing: 8,
                          runSpacing: 6,
                          children: allocationOptions.map((alloc) {
                            final sel = selectedAllocation == alloc;
                            return GestureDetector(
                              onTap: () => setModal(
                                () => selectedAllocation = sel ? null : alloc,
                              ),
                              child: AnimatedContainer(
                                duration: const Duration(milliseconds: 150),
                                padding: const EdgeInsets.symmetric(
                          ...
                          onPressed: () async {
                            if (descCtrl.text.isEmpty ||
                              ...
                              ScaffoldMessenger.of(context).showSnackBar(
                                const SnackBar(
                                  content: Text(
                                    'Keterangan dan jumlah wajib diisi',
                                  ),
                                ),
                              );
                              return;
                            ...
                            final data = {
                              ...
                              'allocation': selectedType == 'expense'
                                  ? selectedAllocation
                                  : null,
                            };
                            Navigator.pop(ctx2);
                            bool ok;
                            if (isEdit) {
                              ok = await _api.updateFinanceEntry(
                                entry['id'],
                                data,
                              );
                            } else {
                            ...
                            if (ok) {
                              ...
                              if (!isEdit) {
                                NotificationService().showFinanceNotification(
                                  ...
                                  body:
                                      '${descCtrl.text.trim()} - ${selectedType == 'income' ? 'Pemasukan' : 'Pengeluaran'}',
                                );
                              }
                              _loadAll();
                            } else {
                ...
                child: SingleChildScrollView(
                  child: Column(
                    ...
                    children: [
                      ...
                      const SizedBox(height: 8),
                      Text(
                        'Set nama dan persentase alokasi sesuai kebutuhan',
                      ...
                      TextButton.icon(
                        onPressed: () {
                          setModalState(() {
                            ...
                            colors.add(
                              defaultColors[labelCtrls.length %
                        ...
                        child: ElevatedButton(
                          ...
                          onPressed: () async {
                            ...
                            for (int i = 0; i < labelCtrls.length; i++) {
                              newAllocs.add({
                                'label': labelCtrls[i].text.trim().isEmpty
                                    ? 'Alokasi ${i + 1}'
                            ...
                            await CacheService.saveFinanceAllocations(
                              newAllocs,
                            );
  ...
  Widget _buildProfitAllocation() {
    ...
    final allocationSpent =
        (_summary['allocation_spent'] as Map<String, dynamic>?) ?? {};

    final totalPct = _allocations.fold<int>(
      0,
      (prev, a) => prev + (a['pct'] as int),
    );
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          Text(
            'Berdasarkan filter: ${_summaryDateRange != null
                ? _displayRange(_summaryDateRange!)
                : _filterSummary == 'daily'
                ? 'Hari Ini'
                : _filterSummary == 'weekly'
                ? 'Minggu Ini'
                : 'Bulan Ini'}',
          ...
          if (_isAllocationExpanded) ...[
            ...
            ..._allocations.map((a) {
              ...
              final spentRatio = allocated > 0
                  ? (spent / allocated).clamp(0.0, 1.5)
                  : 0.0;
                      ...
                      children: [
                        Expanded(
                          child: Column(
                            ...
                            children: [
                              Text(
                                'Anggaran: ${AppFormat.currency(allocated)}',
                                style: TextStyle(
                                  fontSize: 11,
                              ...
                              Text(
                                'Terpakai: ${AppFormat.currency(spent)}',
                                style: TextStyle(
                                  fontSize: 11,
                                  color: spent > allocated && allocated > 0
                                      ? Colors.red[600]
                                      : Colors.grey[600],
                                  fontWeight: FontWeight.w500,
                                ),
      ...
      children: [
        ...
        SingleChildScrollView(
          ...
          child: Row(
            children: [
              ...
              ...filters.map((f) {
                final sel =
                    _summaryDateRange == null && _filterSummary == f['val'];
                return Padding(
              ...
              GestureDetector(
                ...
                child: AnimatedContainer(
                  ...
                  decoration: BoxDecoration(
                    color: _summaryDateRange != null
                        ? const Color(0xFF5D4037)
                        : Colors.white,
                    borderRadius: BorderRadius.circular(20),
                    boxShadow: _summaryDateRange != null
                        ? [
                            BoxShadow(
                              color: Colors.brown.withOpacity(0.2),
                              blurRadius: 6,
          ...
          Padding(
            ...
            child: Row(
              children: [
                ...
                const SizedBox(width: 4),
                Text(
                  'Ringkasan: ${_displayRange(_summaryDateRange!)}',
        ...
        Container(
          ...
          decoration: BoxDecoration(
            ...
            boxShadow: [
              BoxShadow(
                color: (net >= 0 ? Colors.green : Colors.red).withOpacity(0.25),
                blurRadius: 12,
                offset: const Offset(0, 4),
              ),
            ],
  ...
  Widget _buildChartSection() {
    ...
    final incomes =
        (_chartData['incomes'] as List?)
            ?.map((e) => (e as num).toDouble())
            .toList() ??
        [];
    final expenses =
        (_chartData['expenses'] as List?)
            ?.map((e) => (e as num).toDouble())
            .toList() ??
        [];
    ...
    int visibleCount = _chartPeriod == 'daily'
        ? 7
        : (_chartPeriod == 'weekly' ? 8 : 6);
    final visibleLabels = labels.length > visibleCount
        ? labels.sublist(labels.length - visibleCount)
        : labels;
    final visibleIncomes = incomes.length > visibleCount
        ? incomes.sublist(incomes.length - visibleCount)
        : incomes;
    final visibleExpenses = expenses.length > visibleCount
        ? expenses.sublist(expenses.length - visibleCount)
        : expenses;
              ...
              SizedBox(
                ...
                child: Row(
                  ...
                  children: List.generate(visibleLabels.length, (i) {
                    final inc = i < visibleIncomes.length
                        ? visibleIncomes[i]
                        : 0.0;
                    final exp = i < visibleExpenses.length
                        ? visibleExpenses[i]
                        : 0.0;
                    return Expanded(
                      child: Padding(
